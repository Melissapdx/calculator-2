...
while True:
    ...
    if tokens[0] == "q":
        ...
        print "{:.2f}".format(my_reduce(add, make_list_into_float(tokens[0::2])))
        #answers.write("{:.2f} \n".format(my_reduce(add, make_list_into_float(tokens[0::2]))))
    elif tokens[1] == "-":
        print "{:.2f}".format(my_reduce(subtract, make_list_into_float(tokens[0::2])))
        #answers.write("{:.2f} \n".format(my_reduce(subtract, make_list_into_float(tokens[1:]))))
    elif tokens[1] == "*":
        print "{:.2f}".format(my_reduce(multiply, make_list_into_float(tokens[0::2])))
        #answers.write("{:.2f} \n".format(my_reduce(multiply, make_list_into_float(tokens[1:]))))
    elif tokens[1] == "/":
        print "{:.2f}".format(my_reduce(divide, make_list_into_float(tokens[0::2])))
        #answers.write("{:.2f} \n".format(my_reduce(divide, make_list_into_float(tokens[1:]))))
    elif tokens[1] == "square":
        print "{:.2f}".format(square(float(tokens[0::2])))
        #answers.write("{:.2f} \n".format(square(float(tokens[1]))))
    elif tokens[1] == "cube":
        print "{:.2f}".format(cube(float(tokens[0::2])))
        #answers.write("{:.2f} \n".format(cube(float(tokens[1]))))
    elif tokens[1] == "pow":
        print "{:.2f}".format(my_reduce(power, make_list_into_float(tokens[0::2])))
        #answers.write("{:.2f} \n".format(my_reduce(power, make_list_into_float(tokens[1:]))))
    elif tokens[1] == "mod":
        print "{:.2f}".format(my_reduce(mod, make_list_into_float(tokens[0::2])))
